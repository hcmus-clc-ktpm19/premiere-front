...
import { CreateInternalTransactionStep1 } from '@/app/modules/profile/components/steps/CreateInternalTransactionStep1';
import { CreateInternalTransactionStep2 } from '@/app/modules/profile/components/steps/CreateInternalTransactionStep2';
...
const CreateInternalTransaction: React.FC = () => {
  const navigate: NavigateFunction = useNavigate();
  const stepperRef = useRef<HTMLDivElement | null>(null);
  const stepper = useRef<StepperComponent | null>(null);
  const [currentSchema, setCurrentSchema] = useState(
    profileService.transactionValidationSchemas[0]
  );
  ...
  const prevStep = () => {
    ...
    setCurrentSchema(
      profileService.transactionValidationSchemas[stepper.current.currentStepIndex - 1]
    );
  };
      ...
      if (!checking) {
        ...
        try {
          ...
          if (!receivers.find((receiver) => receiver.cardNumber === values.receiverCardNumber)) {
            ...
            setNotification(
              true,
              'Transfer money success. Redirecting to transactions page...',
              notificationType,
              () => {}
            );
          ...
          {(props: FormikProps<any>) => (
            <Form className='py-20 w-100 w-xl-700px px-9' noValidate id='kt_create_account_form'>
              <div className='current' data-kt-stepper-element='content'>
                <CreateInternalTransactionStep1 formikProps={props} />
              </div>

              <div data-kt-stepper-element='content'>
                <CreateInternalTransactionStep2 formikProps={props} />
              </div>
