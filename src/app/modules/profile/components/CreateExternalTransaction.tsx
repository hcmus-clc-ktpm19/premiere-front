...
import {
  CreateExternalTransactionStep1
} from "@/app/modules/profile/components/steps/CreateExternalTransactionStep1";
import {
  CreateExternalTransactionStep2
} from "@/app/modules/profile/components/steps/CreateExternalTransactionStep2";

const CreateExternalTransaction: React.FC = () => {
  ...
  const submitStep = async (values: TransactionRequestDto | any) => {
    ...
    if (stepper.current.currentStepIndex !== stepper.current?.totalStepsNumber - 1) {
      ...
      if (!checking) {
        ...
        try {
          const res = await profileService.validateTransferMoney(transactionRequestDto); // don't need to change
        ...
        try {
          await profileService.transferMoney(transferMoneyRequestDto); // don't need to change
          setSuccess(true);
          setError(false);
  ...
  const onAddReceiver = (receiver: ReceiverDto) => {
    ...
    profileService.insertReceiverExternal(receiver).then((data: ReceiverDto) => {
            ...
            {(props: FormikProps<any>) => (
                <Form className='py-20 w-100 w-xl-700px px-9' noValidate id='kt_create_account_form'>
                  <div className='current' data-kt-stepper-element='content'>
                    <CreateExternalTransactionStep1 formikProps={props} />
                  </div>

                  <div data-kt-stepper-element='content'>
                    <CreateExternalTransactionStep2 formikProps={props} />
                  </div>
