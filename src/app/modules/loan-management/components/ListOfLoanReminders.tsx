...
import { ReminderDeleteModal } from '@/app/modules/loan-management/loan-reminder-modal/ReminderDeleteModal';
...
import ReminderPaymentModal from '@/app/modules/loan-management/loan-reminder-modal/ReminderPaymentModal';
...
const ListOfLoanReminders = () => {
  ...
  useEffect(() => {
    ...
    profileService
      ...
      .then((data: CreditCardDto) => {
        setCreditCard(data);
        services
          .getLoanRemindersByUserCreditCardNumber(data.cardNumber)
  ...
  const openReminderDeleteModal = () => {
    ...
    services
      .getLoanRemindersByUserCreditCardNumber(creditCard!.cardNumber)
      .then((data: LoanReminderDto[]) => {
        setLoanReminders(data);
      })
      .catch((error) => {
  ...
  const onAgreePaymentHandler = (reminder: LoanReminderDto) => {
    ...
    services
      .validateLoanReminder(reminder.id)
      .then((data: string) => {
              ...
              <table className='table table-row-bordered table-row-gray-100 align-middle gs-0 gy-3'>
                ...
                <tbody>
                  {loanReminders.length > 0 ? (
                    loanReminders
                      .filter((reminder) => reminder.status === status || status === '')
                      .map((reminder: LoanReminderDto) => (
                        <tr key={reminder.id}>
                          <td>
